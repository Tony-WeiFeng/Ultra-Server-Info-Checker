...
#     resultFolder = workPath + '\\' + today
#     if not os.path.exists(resultFolder):
#         os.mkdir(resultFolder)
#     
#     
#     def checkBuildInfo(self,serverURL):
#         buildInfoURL = serverURL + '/ultra/fixed/build-info.json'
#         buildInfoJson = json.dumps(urllib2.urlopen(buildInfoURL).read())
#         commitID = buildInfoJson
# 
#     def __init__(self):
#         '''
#         Constructor
...
#     serverURL = 'https://ultra-qa1.int.bbpd.io'
#     bc = BuildInfoChecker()
#     bc.checkBuildInfo(serverURL)
import urllib2  
import os
import time
import smtplib  
...
workPath = os.path.abspath('.')
qa1Folder = workPath + '/ultra-qa1/'
stgFolder = workPath + '/stg/'
demoFolder = workPath + '/demo/'

folderList = [qa1Folder, stgFolder, demoFolder]

for i in range(len(folderList)):
    if not os.path.exists(folderList[i]):
...
def sendMail():
    data = ['', '', '']
    data[0] = saveId('qa1',qa1Folder)
    data[1] = saveId('stg',stgFolder)
    data[2] = saveId('demo',demoFolder)
    if data[0][5] or data[1][5] or data[2][5]:
        sender = 'Bob.Liu@blackboard.com'  
        receivers = ('tony.feng@blackboard.com')
...
#               <td>%s<font color=%s>(%s)</font></td>
#              </tr>
#             """%(alertFlag[0], data[0][0], data[0][1], alertFlag[1], data[0][2], data[0][3], data[0][4]) if data[0][0] !='' and data[0][2] !='' else """
#             <tr>
#              <td>ultra-qa1</td>
